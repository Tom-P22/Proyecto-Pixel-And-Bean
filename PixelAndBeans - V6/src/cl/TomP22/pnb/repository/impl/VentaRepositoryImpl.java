...
public class VentaRepositoryImpl implements IVentaRepository {
       ...
       Connection conn = null;
        try {
           ...
           try (PreparedStatement ps = conn.prepareStatement(SQL_INSERT, Statement.RETURN_GENERATED_KEYS)) {
                ...
                ps.setObject(4, venta.getFechaHora());
    ...
    public List<ProductoVendido> obtenerTopProductos(LocalDate inicio, LocalDate fin) {
        String sql = "SELECT p.nombre, SUM(vd.cantidad) as cant, SUM(vd.subtotal) as total " +
                     "FROM venta_detalle vd " +
                     "JOIN producto p ON vd.producto_id = p.id " +
                     "JOIN venta v ON vd.venta_id = v.id " +
                     "WHERE v.estado = 'ACTIVA' AND DATE(v.fecha_hora) BETWEEN ? AND ? " +
        ...
        try (Connection conn = DatabaseConnectionFactory.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    pv.setCantidad(rs.getInt("cant"));
                    pv.setTotalGenerado(rs.getDouble("total"));
                    lista.add(pv);
                }
            }
        } catch (SQLException e) {
